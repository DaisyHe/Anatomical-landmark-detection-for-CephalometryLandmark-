...
class ShapeViewer ( object ):
  ...
  def show_shapes(shapes):
    ...
    """
    cv2.namedWindow("Shape Model", cv2.WINDOW_NORMAL)
    ...
    #i = cv.CreateImage((max_x-min_x+20, max_y-min_y+20), cv2.IPL_DEPTH_8U, 3)
    #cv.Set(i, (0, 0, 0))
    i = np.zeros((max_x-min_x+20, max_y-min_y+20),np.uint8)
  ...
  def draw_model_fitter(f):
    cv2.namedWindow("Model Fitter", cv2.WINDOW_NORMAL)
    # Copy image
    ...
    for pt_num, pt in enumerate(f.shape.pts):
      # Draw normals
      cv2.circle(i, (int(pt.x), int(pt.y)), 2, (255,0,0), -1)
    cv2.imshow("Shape Model",i)
    cv2.waitKey()
...
class ModelFitter:
  ...
  def __produce_gradient_image(self, i, scale):
    ...
    size = i.shape
    grey_image = np.zeros(size,np.uint8)

    size = [s/scale for s in size]
    #grey_image_small = cv.CreateImage(size, 8, 1)
    grey_image_small = np.zeros(size , np.uint8)

    cv2.resize(grey_image, grey_image_small.shape)#, interpolation=cv.CV_INTER_NN)
    cv2.resize(grey_image_small, grey_image.shape)#, interpolation=cv.CV_INTER_NN)
    return grey_image
