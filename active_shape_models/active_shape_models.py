...
class ShapeViewer ( object ):
  ...
  def show_shapes(shapes):
    ...
    """
    cv2.NamedWindow("Shape Model", cv2.CV_WINDOW_AUTOSIZE)
    ...
    i = cv2.CreateImage((max_x-min_x+20, max_y-min_y+20), cv2.IPL_DEPTH_8U, 3)
    cv2.Set(i, (0, 0, 0))
    for shape in shapes:
      r = randint(0, 255)
      g = randint(0, 255)
      ...
      for pt_num, pt in enumerate(shape.pts):
        ...
        cv2.Circle(i, (int(pt.x-min_x), int(pt.y-min_y)), 2, (r, g, b), -1)
    cv2.ShowImage("Shape Model",i)
  ...
  def draw_model_fitter(f):
    cv2.NamedWindow("Model Fitter", cv2.CV_WINDOW_AUTOSIZE)
    # Copy image
    i = cv2.CreateImage(cv2.GetSize(f.image), f.image.depth, 3)
    cv2.Copy(f.image, i)
    for pt_num, pt in enumerate(f.shape.pts):
      # Draw normals
      cv2.Circle(i, (int(pt.x), int(pt.y)), 2, (0,0,0), -1)
    cv2.ShowImage("Shape Model",i)
    cv2.WaitKey()
...
class ModelFitter:
  ...
  def __produce_gradient_image(self, i, scale):
    size = cv2.GetSize(i)
    grey_image = cv2.CreateImage(size, 8, 1)

    size = [s/scale for s in size]
    grey_image_small = cv2.CreateImage(size, 8, 1)

    cv2.CvtColor(i, grey_image, cv2.CV_RGB2GRAY)

    df_dx = cv2.CreateImage(cv2.GetSize(i), cv2.IPL_DEPTH_16S, 1)
    cv2.Sobel( grey_image, df_dx, 1, 1)
    cv2.Convert(df_dx, grey_image)
    cv2.Resize(grey_image, grey_image_small)#, interpolation=cv.CV_INTER_NN)
    cv2.Resize(grey_image_small, grey_image)#, interpolation=cv.CV_INTER_NN)
    return grey_image
  ...
  def __get_max_along_normal(self, p_num, scale):
    ...
    img = cv2.CreateImage(cv2.GetSize(self.image), self.g_image[scale].depth, 1)
    cv2.Copy(self.g_image[scale], img)
    #cv.Circle(img, \
...
class ActiveShapeModel:
  ...
  def __check_shapes(self, shapes):
    """ Method to check that all shapes have the correct number of points """
    if shapes:
